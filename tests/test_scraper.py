            ...
            for i in range(3):
                ...
                if base_idx < len(lines):
                    ...
                    if base_idx + 1 < len(lines):
                        ...
                        self.assertTrue(
                            '⭐️' in rating_line or '評価なし' in rating_line,
                            f"評価行が正しくありません: {rating_line}"
                        )
        ...
        for limit in [1, 5, 10]:
            with self.subTest(limit=limit):
                try:
                    ...
                    self.assertEqual(product_count, limit, 
                                   f"期待した商品数({limit})と実際の商品数({product_count})が一致しません")
    ...
    def test_performance(self):
        ...
        try:
            ...
            self.assertLess(elapsed_time, 3.0, 
                          f"処理時間が長すぎます: {elapsed_time:.2f}秒")
